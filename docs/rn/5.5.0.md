...
- **BREAKING:** Class deserializer generic methods now constraint type to `class, new()` (before it was `new()` only). This explicitly prevents using value types as deserialization targets (#698).
