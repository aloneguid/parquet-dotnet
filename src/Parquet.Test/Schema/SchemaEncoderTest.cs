...
public class SchemaEncoderTest {
    ...
    [Fact]
    public void Encode_NativeString() {
        var nativeString = new DataField<string>("native");
        SchemaElement se = SchemaEncoder.Encode(nativeString, new ParquetOptions());

    }

    [Fact]
    public void Encode_RomString() {
        var romString = new DataField<ReadOnlyMemory<char>>("rom");
        SchemaElement se = SchemaEncoder.Encode(romString, new ParquetOptions());
    }
