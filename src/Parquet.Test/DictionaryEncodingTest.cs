...
public class DictionaryEncodingTest : TestBase {
    ...
    public async Task AdaptiveSampling_SkipsHighCardinalityColumn() {
        ...
        options.DictionaryEncodedColumns.Add("highCard");
        await using(ParquetWriter writer = await ParquetWriter.CreateAsync(schema, stream, formatOptions: options)) {
    ...
    public async Task AdaptiveSampling_EnablesLowCardinalityColumn() {
        ...
        options.DictionaryEncodedColumns.Add("lowCard");
        await using(ParquetWriter writer = await ParquetWriter.CreateAsync(schema, stream, formatOptions: options)) {
            using ParquetRowGroupWriter rg = writer.CreateRowGroup();
            await rg.WriteAsync(dataField, data);
        }
    ...
    public async Task PerColumnOverride_ForcesOff() {
        ...
        // Write with dictionary encoding enabled
        var optionsWithDict = new ParquetOptions();
        optionsWithDict.DictionaryEncodedColumns.Add("status");
        await using(ParquetWriter writer = await ParquetWriter.CreateAsync(schema, streamWithDict, formatOptions: optionsWithDict)) {
            using ParquetRowGroupWriter rg = writer.CreateRowGroup();
            await rg.WriteAsync(dataField, data);
        }
        ...
        optionsOverrideOff.DictionaryEncodedColumns.Add("status");
        await using(ParquetWriter writer = await ParquetWriter.CreateAsync(schema, streamWithoutDict, formatOptions: optionsOverrideOff)) {
            using ParquetRowGroupWriter rg = writer.CreateRowGroup();
            await rg.WriteAsync(dataField, data);
        }
    ...
    public async Task PerColumnOverride_ForcesOnWhenNotInSet() {
        ...
        // Write without dictionary encoding (column not in DictionaryEncodedColumns)
        await using(ParquetWriter writer = await ParquetWriter.CreateAsync(schema, streamNoDict)) {
            using ParquetRowGroupWriter rg = writer.CreateRowGroup();
            await rg.WriteAsync(dataField, data);
        }

        // Write with per-column override forcing dictionary ON (without being in DictionaryEncodedColumns)
        var optionsOverrideOn = new ParquetOptions {
            ColumnDictionaryEncodings = new Dictionary<string, bool> { ["status"] = true }
        };
        await using(ParquetWriter writer = await ParquetWriter.CreateAsync(schema, streamOverrideOn, formatOptions: optionsOverrideOn)) {
            using ParquetRowGroupWriter rg = writer.CreateRowGroup();
            await rg.WriteAsync(dataField, data);
        }
    ...
    public async Task MixedCardinality_RoundtripWithAdaptiveSampling() {
        ...
        options.DictionaryEncodedColumns.Add("status");
        options.DictionaryEncodedColumns.Add("id");
        await using(ParquetWriter writer = await ParquetWriter.CreateAsync(schema, stream, formatOptions: options)) {
            using ParquetRowGroupWriter rg = writer.CreateRowGroup();
    ...
    public async Task SamplingDisabled_UsesFullScan() {
        ...
        optionsSampled.DictionaryEncodedColumns.Add("col");
        await using(ParquetWriter writer = await ParquetWriter.CreateAsync(schema, streamSampled, formatOptions: optionsSampled)) {
            using ParquetRowGroupWriter rg = writer.CreateRowGroup();
            await rg.WriteAsync(dataField, data);
        }
        ...
        optionsNoSample.DictionaryEncodedColumns.Add("col");
        await using(ParquetWriter writer = await ParquetWriter.CreateAsync(schema, streamNoSample, formatOptions: optionsNoSample)) {
            using ParquetRowGroupWriter rg = writer.CreateRowGroup();
            await rg.WriteAsync(dataField, data);
        }
